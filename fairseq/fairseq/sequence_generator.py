...
class Cure_SequenceGenerator(object):
    def __init__(
        self, models,tokenizer, beam_size=1, minlen=1, maxlen=None, stop_early=True,
        normalize_scores=True, len_penalty=1, unk_penalty=0, retain_dropout=False,
        sampling=False, sampling_topk=-1, sampling_temperature=1,
        diverse_beam_groups=-1, diverse_beam_strength=0.5,
    ):
        ...
        if sampling:
            ...
            self.search = search.Cure_DiverseBeamSearch(tokenizer, diverse_beam_groups, diverse_beam_strength)
        else:
            self.search = search.Cure_BeamSearch(tokenizer)
    ...
    def generate_batched_itr(
        ...
        for sample in data_itr:
            ...
            if "src_tokens" not in encoder_input.keys():
                srclen = encoder_input['ctx_tokens'].size(1)
            else:
                srclen = encoder_input['src_tokens'].size(1)
            if timer is not None:
                timer.start()
    ...
    def _generate(self, encoder_input, beam_size=None, maxlen=None, prefix_tokens=None):
        """See generate"""
        if "src_tokens" in encoder_input.keys():
            src_tokens = encoder_input['src_tokens']
        else:
            src_tokens = encoder_input['ctx_tokens']
        bsz, srclen = src_tokens.size()
        maxlen = min(maxlen, self.maxlen) if maxlen is not None else self.maxlen
        ...
        for model in self.models:
            ...
            # compute the encoder output for each beam
            gpt_out=model.gpt(encoder_input['ctx_tokens'])
            ctx_mask = encoder_input['ctx_types']
            encoder_out = model.encoder(gpt_out,ctx_mask)
            new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
            new_order = new_order.to(src_tokens.device)
            encoder_out = model.encoder.reorder_encoder_out(encoder_out, new_order)
            encoder_outs.append(encoder_out)
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        with torch.no_grad():
            if incremental_states[model] is not None:
                prev_gpt=model.gpt(tokens)
                decoder_out = list(model.decoder( prev_gpt,encoder_out, incremental_state=incremental_states[model]))
            else:
                prev_gpt = model.gpt(tokens)
                decoder_out = list(model.decoder( prev_gpt,encoder_out))
            decoder_out[0] = decoder_out[0][:, -1, :]
            attn = decoder_out[1]
            if type(attn) is dict:
                attn = attn['attn']
