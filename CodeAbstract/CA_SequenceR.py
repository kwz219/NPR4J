...
def run_SequenceR_abs(inputcode_f,outputcode_f,buginfo,max_length):
    ...
    if not os.path.exists(inputcode_f):
    ...
    try:
        ...
        code,hitflag = add_buggy_method_new(class_content,buginfo,max_length)
        print("abs hitflag",hitflag)
        if hitflag==3:
            print(outputcode_f)
    except:
        code=''
...
def add_buggy_method_new(cont,res,max_length):
    buggy_code=res["buggy_code"]
    buggy_line=res["buggy_line"].strip()
    err_start=int(res['err_start'])
    err_end=int(res['err_end'])
    ...
    print("err_start",err_start)
    print("err_end",err_end)
    new_buggy_method=buggy_code[:err_start]+ [error_line] + buggy_code[err_end:]

    m_start_ind=0
    for ind,line in enumerate(new_buggy_method):
        line =str(line.strip())
        if line=="":
            continue
        elif line[0].isalpha():
    ...
    buggycode=new_buggy_method[m_start_ind:]
