    ...
    if method=="SequenceR":
        def build(src_f, tgt_f, error_f, correct_f,ids):
            ...
            for id in ids:
                ...
                try:
                    toked_code=javalang.tokenizer.tokenize(fix_code)
                    toked_fix=''.join([tok.value for tok in toked_code])
                except:
                    toked_code=nltk.word_tokenize(fix_code)
                    toked_fix=''.join(toked_code)

                buggy_code,hitflag=run_SequenceR_abs(input_dir+buggy_parent_f,tmp_f,bug,max_length=1000)
                print(hitflag,"hitflg")
