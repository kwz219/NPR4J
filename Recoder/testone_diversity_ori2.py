...
def testone(ssize, testmodel, filepath, lineid, classname, classcontent_file):
    ...
    for ind,p in enumerate(ans):
        ...
        if lnum == 1 and 'if' in code and mode == 1:
            ...
            tokens = javalang.tokenizer.tokenize(tmpcode)
            parser = javalang.parser.Parser(tokens)
        tans.append({'code': tmpcode,'idx':ind})
    return tans
...
def fix_diversity(model_path,ids_f,input_dir,search_size,fix_dir,classcontent_dir):
    model=load_model_for_test(model_path)
    diversity_ids=codecs.open(ids_f,'r',encoding='utf8').readlines()

    print("total "+str(len(diversity_ids))+" bugs")
    diversity_failed=[]
    sub_ids=diversity_ids[6000:8000]
    for i,id in enumerate(sub_ids):
        print("diversity "+str(i))
        id=id.strip()
        buggyfile=input_dir+'/buggy_methods/'+id+".txt"
        metas=codecs.open(input_dir+'/metas/'+id+'.txt').read()
        ...
        if os.path.exists(fix_dir+'/'+id+'.fix'):
            print("exists")
            continue
        else:
            try:
                ...
                # get patch_code
                for fix in fixs:
                    fix_dict[fix['idx']]=fix["code"]
                patch_f=codecs.open(fix_dir+'/'+id+'.fix','w',encoding='utf8')
                patch_f.write(json.dumps(fix_dict))
            except:
                print("failed")
                diversity_failed.append(id+'\n')


    diverse_failed_f=codecs.open(fix_dir+'/diversity_failed2.txt','w',encoding='utf8')

    diverse_failed_f.writelines(diversity_failed)
...
if __name__ == '__main__':
    import argparse
    fix_diversity("/root/zwk/Recoder_models/best_model.ckpt","/root/zwk/NPR_DATA0306/Evaluationdata/Diversity-processed/Recoder/final.ids","/root/zwk/NPR_DATA0306/Evaluationdata/Diversity",150,
                  "/root/zwk/NPR_DATA0306/FixResults/Diversity_ori","/root/zwk/NPR_DATA0306/Evaluationdata/Diversity-processed/Recoder")
    """"
