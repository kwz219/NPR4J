...
from CoCoNut.tokenization.tokenization import extract_strings, COMPOSED_SYMBOLS, camel_case_split, number_split, \
    remove_integer, token2statement
...
def test_CoCoNut():
    string="entity = clickEvent.getRightClicked('ss');int i=4;"
    toked_str=tokenize(string)
    print(toked_str)
    strs,nums=get_strings_numbers(string)
    print(strs,nums)
    statement=token2statement(toked_str,strs,nums)
    print(statement)
test_CoCoNut()
