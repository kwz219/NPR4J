...
def exclude():
    ori_ids=readF2L("D:\\DDPR_DATA\\OneLine_Replacement\\Raw\\trn_max1k.ids")
    ...
    def getids(ids):
        ...
        return newids
    exclude_ids=list(set(getids(trn_ids)+getids(trn_ids2)+getids(trn_ids3)))
    filtered_ids=[]
    for id in ori_ids:
        if id not in exclude_ids:
            filtered_ids.append(id)
            print(len(filtered_ids))
    writeL2F(filtered_ids,"D:\\DDPR_DATA\\OneLine_Replacement\\Raw\\trn_max1k_f1-3.ids")
